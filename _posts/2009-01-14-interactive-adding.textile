...
Soms is simpelweg toevoegen met @git add .@ of @git commit -am@ niet genoeg. Je wilt je wijzigingen splitsen over meerdere @commits@, of je bent niet zover dat je all je wijzigingen wil toevoegen. En wie wil er nu individuele bestanden een voor een toevoegen? Dat is gewoon saai. Entree voor de interactieve toevoeger: 

@git add -i@

Laten we door een voorbeeld van het gebruik van de interactieve modus heen lopen. Ik heb wat wijzigingen aan het "gitready":http://github.com/qrush/gitready project zijn readme en eeen ander bestand gemaakt. Het gebruiken van het bovengenoemde commando geeft de volgende output, de status van de index:
...
Wanneer je klaar bent, kun je deze modus verlaten met het @quit@ commando, en je werk @committen@. Wanneer je de toevoeger niet vertrouwd, zal @git status@ je laten zien dat uitsluitend readme @staged@ voor @commit@ is:
